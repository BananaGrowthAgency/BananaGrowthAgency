    ...
    <section id="use-cases" className="relative px-4 py-20 md:py-28">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-14 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {USE_CASES.map((u, i) => (
            <Reveal key={`${u.channel}-${i}`} delay={(i % 3) * 0.08}>
              <div className="group flex h-full flex-col rounded-2xl border border-white/10 bg-ink-soft/60 p-6 transition-all duration-300 hover:border-pink/40 hover:neon-ring">
                ...
                <p className="mt-4 text-sm text-foreground/55">{u.context}</p>
                <p className="mt-4 font-display text-2xl font-bold leading-tight text-spectrum">
                  {u.headline}
                </p>
